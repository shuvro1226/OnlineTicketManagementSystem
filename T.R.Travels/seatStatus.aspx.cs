...
public partial class seatStatus : System.Web.UI.Page
{
    SqlConnection con;
    SqlCommand cmd;
    SqlDataReader rea;
    ...
    protected void Changed(object sender, EventArgs e)
    {
        ...
        while (rea.Read())
        {
            for (int j = 1; j <= 31; j++)
            {
                ...
                {
                    ImageButton img = (ImageButton)pnl1.FindControl(s.ToString());
                    img.ImageUrl = "~/images/booked_seat_img.png";
                    img.Enabled = false;
    ...
    public void DataInsert(string PNR, string c_name, string c_phone, string destination, string c_from, string p_time, string totalseat, string seat_num, string amount, string p_date, string creditnum, string securitycode)
    {
        SqlConnection con = new SqlConnection(GetConnectionString());
        string sql = "INSERT INTO passengerinfo(PNR,c_name,c_phone,destination,c_from,p_time,totalseat,seat_num,amount,p_date,creditnum,securitycode)" + "VALUES(@PNR,@C_name,@C_phone,@Destination,@C_from,@P_time,@Totalseat,@Seat_num,@Amount,@P_date,@Creditnum,@Securitycode)";
        try
        {
            ...
            para[10] = new SqlParameter("@Creditnum", SqlDbType.VarChar, 20);
            para[10].Value = creditnum;
            cmd.Parameters.Add(para[10]);
            para[11] = new SqlParameter("@Securitycode", SqlDbType.VarChar, 10);
            para[11].Value = securitycode;
    ...
    {
        if (RadioButton1.Checked == true && txtname.Text.Trim().Length != 0 && txtphone.Text.Trim().Length != 0)
        {
    ...
    {
        if (RadioButton2.Checked == true && txtname.Text.Trim().Length != 0 && txtphone.Text.Trim().Length != 0)
        {
            paychkpnl.Visible = true;
            cardimage.Visible = false;
            lblError.Text = " ";
    ...
    protected void checkButton_Click(object sender, EventArgs e)
    {
        ...
        {
            string cardsql, cardsql1;
            int anInteger, anInteger1;
            int rowsAffected;
            paychkpnl.Visible = false;

            cardsql = "Select * from cust_card where ((namecard_visa ='" + cardnamebox.Text.ToString() + "') and (card_no_visa = '" + cardnobox.Text.ToString() + "')and (securitycode_visa = '" + seccodebox.Text.ToString() + "'))";
            con = new SqlConnection(GetConnectionString());
            cmd = new SqlCommand();

            con.Open();
            cmd.Connection = con;
            cmd.CommandText = cardsql;
            rea = cmd.ExecuteReader();
            if (rea.HasRows == true)
            {

                anInteger = Convert.ToInt32(littotalrent.Text);
                anInteger = int.Parse(littotalrent.Text);

                while (rea.Read())
                {

                    anInteger1 = rea.GetInt32(3);
                    if (anInteger1 > anInteger)
                    {
                        SqlCommand cmd5;
                        SqlConnection con5;
                        cardsql1 = "Update cust_card set visa_avail_amount =visa_avail_amount -'" + anInteger + "' where card_no_visa = '" + cardnobox.Text.ToString() + "'";
                        con5 = new SqlConnection(GetConnectionString());
                        cmd5 = new SqlCommand();
                        con5.Open();
                        cmd5.Connection = con5;
                        cmd5.CommandText = cardsql1;
                        rowsAffected = cmd5.ExecuteNonQuery();
                        // cmpltpanel.Visible = true;
                        ...
                        DataInsert(litpnr.Text.ToString(), txtname.Text.ToString(), txtphone.Text.ToString(), Session["P_dest"].ToString(), Session["From"].ToString(), Session["Time"].ToString(), littotalseat.Text.ToString(), cheatSeatNumber.Text.ToString(), littotalrent.Text.ToString(), Session["Date"].ToString(), cardnobox.Text.ToString(), seccodebox.Text.ToString());
                        ...
                        Literal7.Text = cheatSeatNumber.Text.ToString();
                    ...
                    else
                    {
                        ...
                        lblError.Text = "Insufficient Balance in Visa Card. Try another card";
                        cardimage.Visible = true;
            ...
            {
                lblError.Text = "You don't have any valid visa card number";
                cardimage.Visible = true;
                RadioButton1.Checked = false;
            }
        ...
        if (RadioButton2.Checked == true)
        {
            ...
            cardsql = "Select * from cust_card where ((namecard_master ='" + cardnamebox.Text.ToString() + "') and (card_no_master = '" + cardnobox.Text.ToString() + "')and (securitycode_master = '" + seccodebox.Text.ToString() + "'))";
            con = new SqlConnection(GetConnectionString());
            cmd = new SqlCommand();
            ...
            if (rea.HasRows == true)
            {
                ...
                while (rea.Read())
                {
                    ...
                    anInteger1 = rea.GetInt32(8);
                    if (anInteger1 > anInteger)
                    {
                        SqlCommand cmd5;
                        SqlConnection con5;
                        cardsql1 = "Update cust_card set master_avail_amount =master_avail_amount -'" + anInteger + "' where card_no_master = '" + cardnobox.Text.ToString() + "'";
                        con5 = new SqlConnection(GetConnectionString());
                        cmd5 = new SqlCommand();
                        con5.Open();
                        cmd5.Connection = con5;
                        ...
                        DataInsert(litpnr.Text.ToString(), txtname.Text.ToString(), txtphone.Text.ToString(), Session["P_dest"].ToString(), Session["From"].ToString(), Session["Time"].ToString(), littotalseat.Text.ToString(), txtseatnumber.Text.ToString(), littotalrent.Text.ToString(), Session["Date"].ToString(), cardnobox.Text.ToString(), seccodebox.Text.ToString());
                        DataInsert(litpnr.Text.ToString(), txtname.Text.ToString(), txtphone.Text.ToString(), Session["P_dest"].ToString(), Session["From"].ToString(), Session["Time"].ToString(), littotalseat.Text.ToString(), txtseatnumber.Text.ToString(), littotalrent.Text.ToString(), Session["Date"].ToString(), cardnobox.Text.ToString(), seccodebox.Text.ToString());
                        busnolitrel.Text = litbusnumber.Text.ToString();
                        Literal1.Text = litpnr.Text.ToString();
                        Literal2.Text = txtname.Text.ToString();
                    ...
                    else
                    {
                        ...
                        lblError.Text = "Insufficient Balance in Master Card. Try another card";
                        cardimage.Visible = true;
            ...
            {
                lblError.Text = "You don't have any valid master card number";
                cardimage.Visible = true;
                RadioButton2.Checked = false;
            }
